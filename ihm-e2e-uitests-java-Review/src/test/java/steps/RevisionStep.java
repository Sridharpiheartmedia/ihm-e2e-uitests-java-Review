...
public class RevisionStep extends WebDriverConfig implements En {
    ...
    public RevisionStep(){
        ...
        Then("Click on Down button beside Clone and Launch Revision",()-> revpage.LaunchRevision());
        And("Click on Revise Order in Ad+Orders page",()-> revpage.Addplusorders());
        And("Click Yes on the Popup",()-> revpage.ClickYes());
        And("Click BroadCast Section for Revision",()->{
            revpage.BroadcastRevision();
            revpage.ClickYes();
        });
        And("Click First Station and Add Two Weeks After",()-> revpage.Revision());
        And("Save Changes by clicking Save Only",()-> revpage.SaveOnly());
        And("Return to AddPlusOrders and Submit for Approval",()-> revpage.ReturntoAdPlus());
    }
}
